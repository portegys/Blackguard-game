...
init_globals()
{
	...
	levcount = 0;			/* # of active mons this level */
	levtype = NORMLEV;		        /* type of level this is, maze, etc. */
	trader = 0;				/* no. of purchases */
	curprice = -1;			/* current price of item */
	purse = 0;				/* How much gold the rogue has */
	mpos = 0;				/* Where cursor is on top line */
	packvol = 0;			/* volume of things in pack */
	total = 0;				/* Total dynamic memory bytes */
	demoncnt = 0;			/* number of active daemons */
	lastscore = -1;			/* Score before this turn */
	no_food = 0;			/* # of levels without food */
	count = 0;				/* # of times to repeat cmd */
	fung_hit = 0;			/* # of time fungi has hit */
	quiet = 0;				/* # of quiet turns */
	max_level = 1;			/* Deepest player has gone */
	food_left = HUNGERTIME;	/* Amount of food stomach */
	group = NEWGROUP;		/* Current group number */
	hungry_state = F_OKAY;	/* How hungry is he */
	foodlev = 1;			/* how fast he eats food */
	ringfood = 0;			/* rings affect on food consumption */
}
...
init_names()
{
	...
	for (i = 0; i < MAXSCROLLS; i++) {
		...
		*--cp = '\0';
		s_names[i] = alloc(strlen(prbuf)+1);
#if ANDROID || METRO
		s_know[i] = TRUE;
#else
		s_know[i] = FALSE;
