...
#include <term.h>
#if NCURSES_EXPANDED
 void __attribute__((__cdecl__)) _nc_toggle_attr_on (attr_t *S, attr_t at)
{
 { if (PAIR_NUMBER(at) > 0) { (*S) = ((*S) & ALL_BUT_COLOR) | (at); } else { (*S) |= (at); } ;};
}
 void __attribute__((__cdecl__)) _nc_toggle_attr_off (attr_t *S, attr_t at)
{
 { if (PAIR_NUMBER(at) > 0) { (*S) &= ~(at|((((1UL) << 8) - 1UL) << ((0) + 8))); } else { (*S) &= ~(at); } ;};
}
 int __attribute__((__cdecl__)) _nc_DelCharCost (int count)
{
 return ((cur_term->type. Strings[105] != 0) ? SP->_dch_cost : ((cur_term->type. Strings[21] != 0) ? (SP->_dch1_cost * count) : 1000000));
}
 int __attribute__((__cdecl__)) _nc_InsCharCost (int count)
{
 ...
 void __attribute__((__cdecl__)) _nc_UpdateAttrs (chtype c)
{
