...
using namespace Blackguard::Common;
...
Windows::Foundation::Size RichTextColumns::MeasureOverride(Windows::Foundation::Size availableSize)
{
	if (RichTextContent == nullptr)
	{
		Windows::Foundation::Size emptySize(0, 0);
		return emptySize;
	}
	...
	while (hasOverflow && maxWidth < availableSize.Width && ColumnTemplate != nullptr)
	{
		...
		Windows::Foundation::Size remainingSize(availableSize.Width - maxWidth, availableSize.Height);
	...
	Windows::Foundation::Size resultingSize(maxWidth, maxHeight);
...
Windows::Foundation::Size RichTextColumns::ArrangeOverride(Windows::Foundation::Size finalSize)
{
	float maxWidth = 0;
	float maxHeight = 0;
	...
	while (childrenIterator->HasCurrent)
	{
		...
		Windows::Foundation::Rect childRect(maxWidth, 0, child->DesiredSize.Width, finalSize.Height);
